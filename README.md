...
WPS 365 官方 CLI 工具 — 面向开发者与 AI Agent 的命令行入口。覆盖日历、即时通讯、待办、通讯录、邮箱、云文档、电子表格、多维表、会议等 9 大业务域，提供 200+ 精装命令，未覆盖的接口通过 `api get|post` 直接访问。

[安装](#安装与快速开始) · [命令体系](#双轨命令体系) · [认证](#认证) · [进阶用法](#进阶用法) · [安全](#凭证与安全) · [开发](#开发) · [贡献](#贡献)

## 为什么选 wps365-cli？

- **覆盖面广** — 9 大业务域、200+ 精装命令，一个工具操作整个 WPS 365 平台
- **双轨架构** — 精装命令（语义化、参数友好）+ `api get|post`（全 API 覆盖），按需选择粒度
- **本地 spec 驱动** — 命令来源于本地 OpenAPI 与 curated YAML，支持自定义扩展与覆盖
...
| ✅ 待办 | 个人任务与协作任务的增删改查、状态更新、成员管理、分类管理、批量操作 |
...
| 📊 电子表格 | 工作表管理、单元格读写/查找/删除、行插入、筛选、数据验证、工作表保护 |
...
```bash
go build -o ./wps365-cli ./cmd/wps365-cli
```
...
wps365-cli todo personal-tasks batch-get --status todo --page-size 50
wps365-cli drive files list --drive-id <id> --page-size 20
wps365-cli sheets data get --file-id <id> --sheet-id <id> --range "A1:C10"
```
